...
**功能描述**：资源平台，获取资源仓库某一数据的指定关联关系字段数据列表，包括 `opsany_cmdb_get_resource_fields` 接口中 `is_relationship_field=2` 的字段。
**请求参数**：
...
**功能描述**：资源平台，添加资源的关联关系字段数据时，拉取指定关联关系字段的待添加数据列表（拉取关联关系对端模型数据，已过滤添加过的数据），包括 `opsany_cmdb_get_resource_fields` 接口中 `is_relationship_field=2` 的字段。
**请求参数**：
...
**功能描述**：资源平台，资源仓库给指定数据添加关联关系字段数据，`opsany_cmdb_get_resource_fields` 接口中 `is_relationship_field=2` 的字段为关联关系，使用该工具操作。
**请求参数**：
...
**功能描述**：资源平台，资源仓库给指定数据移除关联关系字段数据，`opsany_cmdb_get_resource_fields` 接口中 `is_relationship_field=2` 的字段为关联关系，使用该工具操作。
**请求参数**：
...
| **attribute** | Object | **是** | - | 属性配置（默认值、提示、校验规则、下拉选项等），不同字段类型格式不同，详见附录 100.1 字段说明。 |
...
| **attribute** | Object | **是** | - | 属性配置，固定格式（提示、默认值、校验规则、下拉框数据等）详见附录 100.1 字段说明。 |
...
| `chname` | String | **是** | 中文名。 |
| `password` | String | **是** | 密码（8-20 位，大小写字母+数字）。 |
| `phone` | String | 否 | 手机号。 |
| `email` | String | 否 | 邮箱。 |
...
**功能描述**：根据作业 ID 执行作业，返回任务 ID。可根据返回的 `flag` 字段判断是否执行完成（`True`: 完成，`False`: 未完成）。
**请求参数**：
...
| **ip_search** | String | 否 | - | 根据主机 IP 模糊搜索纳管主机。 |
| **id** | Integer | 否 | - | 根据主机 ID 查询纳管主机，精准查询。 |
| **host_name** | String | 否 | - | 根据主机唯一标识查询纳管主机，精准查询。 |
| **show_name** | String | 否 | - | 根据主机显示名查询纳管主机，精准查询。 |
| **ip** | String | 否 | - | 根据主机 IP 查询纳管主机，精准查询。 |
...
| 参数名 | 类型 | 必填 | 默认值 | 说明 |
| :--- | :--- | :--- | :--- | :--- |
| **name** | String | 否 | - | 根据控制器名称搜索。 |
| **id** | Integer | 否 | - | 根据控制器 ID 获取详情。 |
...
| 参数名 | 类型 | 必填 | 默认值 | 说明 |
| :--- | :--- | :--- | :--- | :--- |
| **zabbix_id** | String | **是** | - | 通过 `opsany_control_get_zabbix_list` 获取的 Zabbix 实例 ID。 |
...
| 字段 | 类型 | 必填 | 默认值 | 说明 |
| :--- | :--- | :--- | :--- | :--- |
| `name` | String | **是** | - | 主机唯一标识。 |
| `show_name` | String | **是** | - | 主机显示名。 |
| `ip` | String | **是** | - | 主机 IP 地址。 |
| `system_type` | String | **是** | - | 操作系统，仅支持 `Linux` 或 `Windows`。 |
| `controller_id` | Integer | **是** | - | 控制器 ID（通过 `opsany_control_get_controller_list` 获取）。 |
| `control_type` | Integer | **是** | - | 管控方式：`1` SSH, `2` Agent, `3` SSH/Agent, `4` Agent/SSH。 |
| `group_id` | Integer | **是** | - | 主机分组 ID（通过 `opsany_control_get_host_group_list` 获取）。 |
| `host_type` | String | **是** | - | 主机类型：`SERVER`(物理机) 或 `VIRTUAL_SERVER`(虚拟机)。 |
| `username` | String | 否 | `root` | 主机系统用户。 |
| `password` | String | 否 | - | 主机密码。 |

**端口配置：**

| 字段 | 类型 | 必填 | 默认值 | 说明 |
| :--- | :--- | :--- | :--- | :--- |
| `ssh_port` | String | 否 | `22` | SSH 端口，Linux 默认 `22`。 |
| `login_port` | String | 否 | `3389` | RDP 端口，Windows 默认 `3389`。 |

**特权提升（Sudo/Su）：**

| 字段 | 类型 | 必填 | 说明 |
| :--- | :--- | :--- | :--- |
| `privilege` | Boolean | 否 | 是否开启特权提升。 |
| `privilege_type` | String | 否 | 特权类型：`sudo` 或 `su`。 |
| `privilege_username` | String | 否 | 特权用户名。 |
| `privilege_password` | String | 否 | 特权密码。 |

**监控插件配置：**

| 字段 | 类型 | 必填 | 说明 |
| :--- | :--- | :--- | :--- |
| `monitor_type` | String | 否 | 监控类型：`Zabbix` 或 `Prometheus`。 |
| `controller_zabbix` | String | 否 | monitor_type 为 Zabbix 时必填。 |
| `controller_prom` | String | 否 | monitor_type 为 Prometheus 时必填。 |
| `bind_port` | Integer | 否 | Prometheus 自定义端口，默认 `9101`。 |
| `template_list` | Array | 否 | Zabbix 模板列表（包含 `temp_name` 和 `temp_id`）。 |
| `dashboard_dict` | Object | 否 | 大屏信息（`uid`, `title`, `url`, `tags`）。 |
...
| `attribute.rule_id` | `form2.rule_id` | string\|number | 校验规则 ID（`"empty"` 表示无校验） |
| `attribute.rule` | `form2.rule` | object | 校验规则 `{ re: "正则表达式" }` |

---

#### str（单行文本）/ textarea（多行文本）/ link（链接）
...
| 字段 | 来源 | 类型 | 说明 |
| :--- | :--- | :--- | :--- |
| `attribute.rule_id` | `form2.rule_id` | string\|number | 校验规则 ID |
...
| 字段 | 来源 | 类型 | 说明 |
| :--- | :--- | :--- | :--- |
| `attribute.富文本` | `form2["富文本"]` | string | 富文本 HTML 内容 |
| `attribute.用户提示` | `form2["用户提示"]` | string | 占位提示文本 |
...
| 字段 | 来源 | 类型 | 说明 |
| :--- | :--- | :--- | :--- |
| `attribute.选项` | `dropDownList` | `[{name, id}]` | 选项列表（`name`=选项名/显示名，`id`=选项值/唯一标识，id 使用变量格式，理论上不可修改） |
| `attribute.用户提示` | `form2["用户提示"]` | string | 占位提示文本 |
...
注意：`composite_type`（1=手动新建，2=从模型导入）仅用于 UI 交互，不会发送到后端。
...
| QQ 号码 | str | `[1-9]([0-9]{4,10})` |
| IP 地址 | str | `^((25[0-5]\|2[0-4]\d\|[1]{1}\d{1}\d{1}\|[1-9]{1}\d{1}\|\d{1})($\|(?!\.$)\.)){4}$` |
| 正整数 | int | `[1-9][0-9]*` |
| 负整数 | int | `-[1-9][0-9]*` |
| 整数 | int | `-?[1-9][0-9]*` |
...
- `username`：OpsAny 当前用户名
- `user-api-token`：OpsAny 工作台 → 个人设置 → API Token
- `mcp-auth-token`：config.yaml 中 `auth_token` 值

#### 常见问题排查

**连接失败：** 确认 MCP Server 正在运行，检查 URL 是否正确（应为 `http://host:8020/sse`），查看服务器日志。

**认证失败：** 检查 `config.yaml` 中的 API 凭证，确认 OpsAny 平台地址可访问，验证用户权限是否足够。

**数据返回为空：** 确认 OpsAny 平台中有相关资源数据，检查搜索关键词是否正确，尝试不使用搜索参数获取全部数据。
